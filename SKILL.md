...
全自动 Amazon 选品评论分析 pipeline：**仅在 Agent 引导用户确认后**，用户明确说"要/好的/去抓/跑一下"时才执行**。执行链路：提取品类+翻译关键词 → 问三个参数 → 执行 pipeline.py → 输出报告 → 完成。
...
**触发后，先提取品类和关键词，然后必须问以下三个问题，等用户回复后再执行 pipeline。**
